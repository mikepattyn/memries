...
name: platform-lint
description: >-
  Orchestrates linting for Memries remainder trees: e2e/ and scripts/. Diffs
...
Reusable **orchestrator**. Trees are the explicit list in [`scripts/app-fanout/app-fanout.config.json`](../../../scripts/app-fanout/app-fanout.config.json) (`platform-lint.trees`): `e2e` and `scripts`.

Planning uses [`scripts/app-fanout/app-fanout.mjs`](../../../scripts/app-fanout/app-fanout.mjs) (`--skill platform-lint`).
...
```
node scripts/app-fanout/app-fanout.mjs plan --skill platform-lint
```
...
```
node scripts/app-fanout/app-fanout.mjs close --skill platform-lint e2e
node scripts/app-fanout/app-fanout.mjs record --skill platform-lint --commit <base-sha> e2e
```
