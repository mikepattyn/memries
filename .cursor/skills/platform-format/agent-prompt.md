...
- `e2e` / `scripts`: Prettier for JS/TS. If no Prettier, copy `.cursor/skills/app-fanout/assets/.prettierrc`.
- Do **not** add shfmt. Do **not** replace existing formatter config.
...
2. Stage only `{{PATH}}`. Never stage `.cursor/skills/platform-format/last-runs.json`.
3. Message like `style({{ID}}): format with prettier`.
