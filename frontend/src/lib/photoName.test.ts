...
describe("photoName", () => {
  ...
  it("names a timeline Photo control from TakenAt", () => {
    ...
    expect(photoOpenLabel({ ...photo, favorite: true })).toBe(`Open favorited photo, ${photoWhen(photo)}`);
  });

  it("describes the Original in the photo viewer", () => {
    expect(photoViewerAlt(photo)).toBe(`Photo, ${photoWhen(photo)}`);
    expect(photoViewerAlt({ ...photo, favorite: true })).toBe(`Photo, ${photoWhen(photo)}, favorited`);
  });
});
