...
3. Images use repo-root [Dockerfile.frontend](../../Dockerfile.frontend) and [Dockerfile.backend](../../Dockerfile.backend). Each runs `turbo prune --docker`, then [docker/fill-prune-output.mjs](../../docker/fill-prune-output.mjs) copies `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `package.json`, `turbo.json`, `.npmrc` (and `go.mod` / `go.sum` for backend) into `out/json` and `out/full`. The backend builder has Node 20 and Go 1.23 and runs `turbo run build --filter=@memries/backend`.
4. Host toolchain install is [apps/scripts/install-requirements/](../../apps/scripts/install-requirements/) (Node 20, Go 1.23, corepack pnpm). It does not install Docker or workspace deps. `make up` remains Compose. `turbo dev` is the Vite SPA only.
